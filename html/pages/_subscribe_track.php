...
$frm = file_get_contents('pages/_subscribe_track.frm');
$opts = '';
...
if ($enddate!='')$enddate = convertFromDateToUnixTimestamp($enddate);
 ...
 $rowsperpage = GetMarketingListCountFor(1,$company,$email,$stdate,$enddate,$product);
...
$no = $first+1;
$articles = GetMarketingListForSource($first,$rowsperpage,1,$company,$email,$stdate,$enddate,$product);
...
//print_r($articles);
$content.= 'Total subscribed: '.GetMarketingListCountFor(1,$company,$email,$stdate,$enddate,$product);
...
foreach($articles as $key => $row){
	...
	$rowhtml =str_replace('@DELLINK@', '?page=_subscribe_track&entryid='.$row['id'].'&action=remove', $rowhtml);
	$table.=$rowhtml;
	$no++;
}
$table.='</table></p>';
...
$totalpages = (int)((GetMarketingListCountFor(1,$company,$email,$stdate,$enddate,$product)+$rowsperpage-1)/$rowsperpage);

$pglinks = '<p class="downloadEdit-pages">';
for ($i = 0; $i < $totalpages; $i++){
	$pglinks.='|<a href="?page=_subscribe_track&pg='.($i+1).'">'.($i+1).'</a>';
}
