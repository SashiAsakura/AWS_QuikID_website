...
//print_r($articles);
$content.= 'Total downloads: '.GetMarketingListCountFor(2,$company,$email,$stdate,$enddate,$product);

$table = "<!--- start list of download -->";
$table.='<table style="border:1 solid black;background-color:white;">';
$table.='<tr style="background-color:#999999;color:white;"><td style="width:40px">No</td><td style="width:200px;">Company</td><td style="width:300px"> Name </td><td style="width:200px"> Email </td><td style="width:200px"> Phone </td><td style="width:40px"> ADV </td><td style="width:230px"> date </td><td style="width:60px"> type </td><td>&nbsp;</td></tr>';
foreach($articles as $key => $row){
	$rowtpl = '<tr class="downloadEdit-list"><td>@NO@</td><td>@COMPANY@</td><td>@NAME@</td><td>@EMAIL@</td><td>@PHONE@</td><td>@ADVERTISMENT@</td><td>@TIME@</td><td>@TYPE@</td><td><a href="@DELLINK@">X</a></td></tr>';
	$nostr = $no;
	...
	$rowhtml =str_replace('@PHONE@', $row['phone'], $rowhtml);
	...
	$rowhtml =str_replace('@DELLINK@', '?page=_download_track&entryid='.$row['id'].'&action=remove', $rowhtml);
	$table.=$rowhtml;
	$no++;
}
$table.='</table></p>';
...
$totalpages = (int)((GetMarketingListCountFor(2,$company,$email,$stdate,$enddate,$product)+$rowsperpage-1)/$rowsperpage);

$pglinks = '<p class="downloadEdit-pages">';
for ($i = 0; $i < $totalpages; $i++){
	$pglinks.='|<a href="?page=_download_track&pg='.($i+1).'">'.($i+1).'</a>';
}
