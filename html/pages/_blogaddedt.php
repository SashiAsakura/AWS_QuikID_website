...
$form = '';
if ($showform)$form = file_get_contents("pages/_blogaddedt.frm");
...
if ($pressid==-1){
	...
	if ($details['image']!=null){
		$img_lnk = '<img src="getblogfile.php?id='.$details['id'].'">';
	}
}
...
if ($pressid!=-1){
	$previewlnk = '<a href="?page=blogdtls&prid='.$pressid.'" target="_blank">preview blog</a>';
}else $previewlnk ='';

$names = array('@TITLE@','@SUBTITLE@','@LEAD@','@BODY@','@BUTTON@','@STATUS@','@PRESSID@','@ACTION@','@DATE@','@MAINPG@','@IMAGE@','@IMGALT@','@META@','@AUTHOR@','@TESTMODE@','@PREVIEWLINK@');
$values= array(htmlspecialchars($title),$subtitle,$lead,$body,$button,$status,$pressid,$action,$date,$mainpage,$img_lnk,htmlspecialchars($imgalt),$meta,$author,$tst_mode,$previewlnk);

$content = str_replace($names, $values, $form);

$scripts = file_get_contents("pages/_blogaddedt.scr");
